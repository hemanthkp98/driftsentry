...
class HTMLFormatter:
    ...
    def _build_html(self, result: DriftResult) -> str:
        ...
        regions_meta = (
            ...
            if result.regions
            else f"Region: {result.region or 'N/A'}"
        )

        meta_parts = [
            f"Scan ID: {result.scan_id}",
            result.iac_tool.value.capitalize(),
            f"Provider: {result.provider}",
        ]
        if accounts_meta:
            meta_parts.append(accounts_meta)
        meta_parts.append(regions_meta)
        meta_parts.append(f"Duration: {result.duration_seconds}s")
        meta_line = " | ".join(meta_parts)

        account_header = "<th>Account</th>" if show_account else ""
        region_header = "<th>Region</th>" if show_region else ""
...
<body>
    ...
    <div class="footer">
        Generated by DriftSentry v0.2.0 — Your infrastructure's immune system
    </div>
</body>
</html>"""
    ...
    def _build_table_rows(
        ...
        for item in result.drift_items:
            ...
            account_td = (
                f"<td><code>{item.account_name or item.account_id or '-'}</code></td>"
                if show_account
                else ""
            )
