...
class PRCreator:
    ...
    def create_pr(
        ...
        if remediation.ai_result and (
            remediation.ai_result.root_causes or remediation.ai_result.hcl_results
        ):
            ai_lines = [
                "",
                "## 🤖 AI Root Cause & Security Insights",
                f"*Generated by AI Engine ({remediation.ai_result.provider_used}: {remediation.ai_result.model_used})*",
                "",
            ]
            for rc in remediation.ai_result.root_causes:
                ai_lines.append(f"### `{rc.resource_address}`")
                ai_lines.append(f"- **Root Cause:** {rc.narrative}")
                if rc.risk_assessment:
                    ai_lines.append(f"- **Risk Assessment:** {rc.risk_assessment}")
                ai_lines.append(f"- **Recommended Action:** `{rc.recommended_action}`")
                ai_lines.append("")

            footer_marker = "\n---\n*Generated by [DriftSentry]"
            if footer_marker in pr_body:
                parts = pr_body.split(footer_marker, 1)
                pr_body = parts[0] + "\n".join(ai_lines) + footer_marker + parts[1]
            else:
                pr_body += "\n" + "\n".join(ai_lines)
